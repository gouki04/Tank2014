#ifndef STRING_UTILITY_H__
#define STRING_UTILITY_H__
...
namespace utility
{
    ...
    {
        if (_right) _str.erase(_str.find_last_not_of(" \n\t\r") + 1);
        if (_left) _str.erase(0, _str.find_first_not_of(" \n\t\r"));
    }
    ...
    inline void split(std::vector<std::string> & out_result, const std::string& _source, const std::string& _delims = "\t\n ")
    {
        //std::vector<std::string> result;
        templates::split<std::string>(out_result, _source, _delims);
        //return result;
    }
